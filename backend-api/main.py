...
async def generate_quiz_endpoint(data: GeneratePDFRequest):
    session_id = data.session_id
    if session_id not in sessions:
    ...
    async with lock:
        try:
            # Generate quiz and get the path
            quiz_path = sessions[session_id].generate_quiz("./outputs/" + session_id + ".pdf")
            return FileResponse(
                path=quiz_path,
